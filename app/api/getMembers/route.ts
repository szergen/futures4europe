...
export const GET = async () => {
  try {
    ...
    try {
      ...
      const queryResult = await wixClientServer.members.queryMembers({
        fieldsets: ['FULL']
      })
      .limit(100)
      .find();
...
export const POST = async (req: NextRequest) => {
  try {
    const { searchType, searchValue, hasNoInfoPage, action, memberId, newHasNoInfoPageValue } = await req.json();
    ...
    if (action === 'updateHasNoInfoPage' && memberId) {
      return handleUpdateLabels(wixClientServer, memberId, newHasNoInfoPageValue);
    }
    
    // Handle search action
    return handleSearchMembers(wixClientServer, searchType, searchValue, hasNoInfoPage);
  } catch (error) {
...
async function handleUpdateLabels(wixClientServer, memberId, newHasNoInfoPageValue) {
  try {
    ...
    try {
      ...
      try {
        ...
        console.log('DEBUG: Label creation error (might already exist):', labelErr.message);
      }
      
      // Determine if we need to add or remove the label
      if (newHasNoInfoPageValue) {
        // Add the label to the contact
        console.log('DEBUG: Adding hasNoInfoPage label to contact', member.contactId);
        ...
        console.log('DEBUG: Removing hasNoInfoPage label from contact', member.contactId);
      ...
      try {
        ...
        const updatedMember = await wixClientServer.members.updateMember(updateData);
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to update member labels', error: String(error) }, 
      { status: 500 }
    );
  }
}
...
async function handleSearchMembers(wixClientServer, searchType, searchValue, hasNoInfoPage) {
  try {
    console.log(`Searching for member by ${searchType}: ${searchValue}, hasNoInfoPage: ${hasNoInfoPage}`);
    ...
    if (searchType === 'id') {
      results = await searchMembersById(wixClientServer, searchValue, hasNoInfoPage);
    } else if (searchType === 'email') {
      results = await searchMembersByEmail(wixClientServer, searchValue, hasNoInfoPage);
    } else if (searchType === 'name') {
      results = await searchMembersByName(wixClientServer, searchValue, hasNoInfoPage);
    } else if (searchType === 'hasNoInfoPage') {
      results = await searchMembersByHasNoInfoPage(wixClientServer, searchValue);
    } else {
...
async function searchMembersByEmail(wixClientServer, email, hasNoInfoPage) {
  try {
    ...
    if (!queryResult.items || queryResult.items.length === 0) {
      ...
      const allMembers = await wixClientServer.members.queryMembers({
        fieldsets: ['FULL']
      })
      .limit(100)
      .find();
      ...
      let filteredMembers = (allMembers.items || []).filter(member => 
        member.loginEmail && member.loginEmail.toLowerCase().includes(email.toLowerCase())
      );
...
async function searchMembersByName(wixClientServer, name, hasNoInfoPage) {
  try {
    ...
    const [firstNameResult, lastNameResult, nicknameResult] = await Promise.all([
      wixClientServer.members.queryMembers({ fieldsets: ['FULL'] })
    ...
    let uniqueMembers = allResults.filter((member, index, self) =>
