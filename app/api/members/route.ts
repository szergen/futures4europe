...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.log("Request body parsed, contactId:", contactId, "action:", action, "revision:", revision);
    } catch (parseError) {
    ...
    try {
      ...
      const hasNoInfoPageLabel = allLabels.some(label => 
        label === 'custom.hasnoinfopage' || 
        label.toLowerCase() === 'custom.hasnoinfopage'
      );
      
      console.log("Does contact have the 'custom.hasnoinfopage' label?", hasNoInfoPageLabel);
      ...
      return NextResponse.json(
        { message: 'Error fetching contact details', error: getContactError.message },
        { status: 404 }
      );
    }
    ...
    try {
      if (action === "addLabel") {
        console.log("Adding label 'custom.hasnoinfopage' to contact", contactId);
        
        // The labelContact method expects an array of label keys
        await wixClient.contacts.labelContact(contactId, ['custom.hasnoinfopage']);
        ...
        const hasLabel = updatedContact.info?.labelKeys?.items?.includes('custom.hasnoinfopage');
        console.log("After addition, does contact have the label?", hasLabel);
        
        if (!hasLabel) {
          console.warn("Label addition reported success but label is not present");
          return NextResponse.json({
            success: false,
        ...
        console.log("Removing label 'custom.hasnoinfopage' from contact", contactId);
        
        // The unlabelContact method expects an array of label keys
        await wixClient.contacts.unlabelContact(contactId, ['custom.hasnoinfopage']);
        
        console.log("Label removal operation completed");
        
        // Verify the label has been removed
        const updatedContact = await wixClient.contacts.getContact(contactId);
        const stillHasLabel = updatedContact.info?.labelKeys?.items?.includes('custom.hasnoinfopage');
        console.log("After removal, does contact still have the label?", stillHasLabel);
        ...
        if (stillHasLabel) {
          ...
          try {
            ...
            const stillHasLabelAfterAlternative = finalContact.info?.labelKeys?.items?.includes('custom.hasnoinfopage');
            ...
            if (stillHasLabelAfterAlternative) {
              ...
              console.log("Alternative approach successfully removed the label");
            }
      ...
      return NextResponse.json(
        { 
          message: `Error ${action === "addLabel" ? "adding" : "removing"} label`, 
    ...
    return NextResponse.json(
      { message: 'Server error while processing request', error: error.message, stack: error.stack },
      { status: 500 }
    );
  }
}