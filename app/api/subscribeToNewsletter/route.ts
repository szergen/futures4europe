...
export const POST = async (req: NextRequest) => {
  ...
  try {
    ...
    const consentResult = await wixClient.marketingConsent.upsertMarketingConsent({
      details: {
        email,
        type: 'EMAIL',
      }
    ...
    try {
      const response = await fetch('https://www.wixapis.com/email-marketing/v1/email-subscriptions', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        try {
          ...
          const submissionResult = await wixClient.submissions.createSubmission(formSubmission);
          ...
          if (submissionResult.status === "PENDING" && submissionResult._id) {
            const confirmedSubmission = await wixClient.submissions.confirmSubmission(submissionResult._id);
