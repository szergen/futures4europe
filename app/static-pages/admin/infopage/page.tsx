...
interface Contact {
  ...
  primaryEmail?: { email: string; subscriptionStatus: string; deliverabilityStatus: string };
  picture?: string;
  info: {
...
const PersonInfoLabeler = () => {
  ...
  const [notification, setNotification] = useState<{ message: string; type: MessageType }>({ message: '', type: '' });
  ...
  const contactsPerPage = 100;
    ...
    useEffect(() => {
        ...
        contacts.forEach(contact => {
            ...
            countBySourceType[sourceType] = (countBySourceType[sourceType] || 0) + 1;
        });
...
const debugMemberDetails = () => {
    ...
    const siteMembers = contacts.filter(contact => 
    ...
    siteMembers.forEach(member => {
      ...
      if (member.info?.extendedFields?.items) {
        Object.entries(member.info.extendedFields.items).forEach(([key, value]) => {
  ...
  const notify = (message: string, type: MessageType, timeout = 5000) => {
    ...
    if (timeout > 0) setTimeout(() => setNotification({ message: '', type: '' }), timeout);
  };
  ...
  // Data fetching
    const fetchContacts = async (signal: AbortSignal, shouldAppend = false) => {
        setIsLoading(true);
        try {
        const response = await fetch(`/api/members?page=${page}&limit=${contactsPerPage}`, { signal });
  ...
  const updateStats = () => {
    ...
    const totalMembers = contacts.filter(contact => 
      contact.source?.sourceType === 'WIX_SITE_MEMBERS'
    ).length;
    
    const contactsWithInfoPage = contacts.filter(contact => {
      const contactFirstName = (contact.info.name?.first || '').toLowerCase().trim();
      const contactLastName = (contact.info.name?.last || '').toLowerCase().trim();
      ...
      return infoPages.some((page) => {
        ...
        if (titleMatches) {
          return page.data?.person && Array.isArray(page.data.person) && page.data.person.length > 0;
        }
    ...
    const membersWithoutInfoPages = contacts.filter(contact => {
        ...
        const hasLabel = contact.info.labelKeys?.items?.includes('custom.hasnoinfopage');
  ...
  const contactsToLabel = useMemo(() => {
    ...
    return contacts
      .filter((contact) => {
        if (filterOnlyMembers && contact.source?.sourceType !== 'WIX_SITE_MEMBERS') return false;
        ...
        if (searchTerm) {
          ...
          if (!contactFullName.includes(searchLower) && 
        ...
        // Apply sort filter
        if (sortOption === 'members' && contact.source?.sourceType !== 'WIX_SITE_MEMBERS') {
          return false;
        } else if (sortOption === 'contacts' && contact.source?.sourceType === 'WIX_SITE_MEMBERS') {
          return false;
        }
  ...
  const startLabelingProcess = async (resume = false) => {
    ...
    const queue: Contact[] = contacts.filter((c) =>
      contactsToLabel.some((ct) => ct._id === c._id && !results.successful.includes(c._id) && !results.failed.includes(c._id))
    );
    ...
    if (initialResults.failed.length > 0) statusMessage += ` Failed for ${initialResults.failed.length} contacts.`;
    ...
    notify(statusMessage.trim(), initialResults.failed.length > 0 ? 'error' : 'success', 15000);
  ...
  const processContact = async (
    ...
    try {
      ...
      const contactResponse = await fetch(`/api/members/contact?id=${contactId}`);
      ...
      if (response.ok && data.success) {
      ...
      } else if (retryCount < 3) { // Enhanced error recovery - increase max retries to 3
        ...
        notify(`Retrying contact ${contactId} (attempt ${retryCount + 1})`, 'info', 2000);
        return processContact(contact, retryCount + 1);
      } else {
        results.failed.push(contactId);
        return { success: false, contactId, error: data.message, retried: retryCount > 0 };
      }
    } catch (error: any) {
      if (retryCount < 3) { // Enhanced error recovery
        // Exponential backoff for retries
        const backoffDelay = 1000 * Math.pow(2, retryCount);
        await new Promise((resolve) => setTimeout(resolve, backoffDelay));
        results.retried = results.retried || [];
      ...
      results.failed.push(contactId);
      return { success: false, contactId, error: error.message, retried: retryCount > 0 };
    }
  };

  const getInfoPageUrl = useCallback((contact) => {
    ...
    const match = infoPages.find(page => {
      ...
      if (!page.data?.person || !Array.isArray(page.data.person)) return false;
          ...
          <div className="mb-6 bg-white rounded-lg shadow-sm p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-blue-50 rounded-md p-3">
                <p className="text-sm text-blue-800 font-medium">Total Contacts</p>
                <p className="text-2xl text-blue-900 font-bold">{stats.totalContacts}</p>
                <p className="text-xs text-blue-700 mt-1">Members: {stats.totalMembers}</p>
              </div>
              <div className="bg-green-50 rounded-md p-3">
                <p className="text-sm text-green-800 font-medium">Contacts with Info Pages</p>
                <p className="text-2xl text-green-900 font-bold">{stats.contactsWithInfoPages}</p>
                <p className="text-xs text-green-700 mt-1">
                  {((stats.contactsWithInfoPages / stats.totalContacts) * 100 || 0).toFixed(1)}% of all contacts
                </p>
              </div>
              <div className="bg-orange-50 rounded-md p-3">
                <p className="text-sm text-orange-800 font-medium">Contacts without Info Pages</p>
                <p className="text-2xl text-orange-900 font-bold">{stats.contactsWithoutInfoPages}</p>
                <p className="text-xs text-orange-700 mt-1">
                  Members: {stats.membersWithoutInfoPages} ({((stats.membersWithoutInfoPages / stats.totalMembers) * 100 || 0).toFixed(1)}% of members)
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6 bg-white rounded-lg shadow-sm p-4">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                disabled={isProcessing}
                >
                <option value="all">All Contacts ({contactsToLabel.length})</option>
                <option value="members">Members Only ({contactsToLabel.filter(ct => 
                    contacts.find(c => c._id === ct._id)?.source?.sourceType === 'WIX_SITE_MEMBERS'
                ).length})</option>
                <option value="contacts">Non-Members Only ({contactsToLabel.filter(ct => 
                    contacts.find(c => c._id === ct._id)?.source?.sourceType !== 'WIX_SITE_MEMBERS'
                ).length})</option>
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  disabled={isProcessing}
                />
                <span className="text-sm font-medium text-gray-700">Process only members</span>
              </div>
            </div>
          </div>
...
{contactsToLabel.length > 0 ? (
  <ul className="divide-y divide-gray-200 max-h-64 overflow-y-auto">
    {contactsToLabel.slice(0, 50).map((contact) => {
      ...
      const fullContact = contacts.find(c => c._id === contact._id);
      const isMember = fullContact?.source?.sourceType === 'WIX_SITE_MEMBERS';
      const sourceType = fullContact?.source?.sourceType || 'Unknown';
      
      // Get subscription and membership status
      const subscriptionStatus = fullContact?.primaryEmail?.subscriptionStatus || 
                                fullContact?.info?.extendedFields?.items?.['emailSubscriptions.subscriptionStatus'] || 
                                'NOT_SET';
      const deliverabilityStatus = fullContact?.primaryEmail?.deliverabilityStatus || 
                                  fullContact?.info?.extendedFields?.items?.['emailSubscriptions.deliverabilityStatus'] || 
                                  'NOT_SET';
      const membershipStatus = fullContact?.info?.extendedFields?.items?.['members.membershipStatus'] || 'N/A';
      
      // Get profile picture
      const profilePicture = fullContact?.picture || fullContact?.info?.picture?.image || null;
      ...
      const matchingInfoPage = infoPages.find(page => {
        const pageTitle = (page.title || '').toLowerCase().trim();
        return pageTitle === contactName || 
               (page.data?.person && Array.isArray(page.data.person) && 
               page.data.person.some(p => (p.name || '').toLowerCase().trim() === contactName));
      });
      
      return (
        <li key={contact._id} className="py-3 flex items-start justify-between hover:bg-gray-50">
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <p className="text-sm font-medium text-gray-800">{contact.name || 'Unnamed Contact'}</p>
                <span className={`px-2 py-0.5 rounded-full text-xs ${
                  sourceType === 'WIX_SITE_MEMBERS' ? 'bg-blue-100 text-blue-800' : 
                  sourceType === 'WIX_FORMS' ? 'bg-green-100 text-green-800' :
                  sourceType === 'WIX_EVENTS' ? 'bg-purple-100 text-purple-800' :
                  sourceType === 'WIX_CHAT' ? 'bg-yellow-100 text-yellow-800' :
                  sourceType === 'WIX_EMAIL_MARKETING' ? 'bg-pink-100 text-pink-800' :
                  sourceType === 'IMPORT' ? 'bg-indigo-100 text-indigo-800' :
                  sourceType === 'THIRD_PARTY' ? 'bg-orange-100 text-orange-800' :
                  sourceType === 'ADMIN' ? 'bg-red-100 text-red-800' :
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {isMember && (
                  <span className={`px-2 py-0.5 rounded-full text-xs ${
                    membershipStatus === 'APPROVED' ? 'bg-green-50 text-green-700 border border-green-200' :
                    membershipStatus === 'OFFLINE_ONLY' ? 'bg-amber-50 text-amber-700 border border-amber-200' :
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs ${
                  subscriptionStatus === 'SUBSCRIBED' ? 'bg-green-50 text-green-700 border border-green-200' :
                  subscriptionStatus === 'UNSUBSCRIBED' ? 'bg-red-50 text-red-700 border border-red-200' :
                  subscriptionStatus === 'PENDING' ? 'bg-yellow-50 text-yellow-700 border border-yellow-200' :
                  'bg-gray-50 text-gray-700 border border-gray-200'
                }`}>
                  {subscriptionStatus}
                </span>
                ...
                {deliverabilityStatus !== 'NOT_SET' && (
                  <span className={`px-2 py-0.5 rounded-full text-xs ${
                    deliverabilityStatus === 'VALID' ? 'bg-green-50 text-green-700 border border-green-200' :
                    deliverabilityStatus === 'BOUNCED' ? 'bg-red-50 text-red-700 border border-red-200' :
                    deliverabilityStatus === 'INACTIVE' ? 'bg-amber-50 text-amber-700 border border-amber-200' :
                    'bg-gray-50 text-gray-700 border border-gray-200'
                  }`}>
          ...
          <div className="flex items-center gap-2">
            {hasInfoPage ? (
              ...
              <span className="inline-flex items-center px-2 py-1 text-xs font-medium rounded-md text-gray-700 bg-gray-100" title="No info page available">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    No contacts eligible for labeling based on current filters.
          ...
          <div className="mb-6">
            <button
              ...
              disabled={isProcessing || isLoading || contactsToLabel.length === 0}
          ...
          {isProcessing && (
            <div className="bg-blue-50 border border-blue-200 rounded-md p-4 mb-6">
              <h3 className="text-sm font-medium text-blue-800 mb-2">Processing contacts...</h3>
          ...
          {/* Results Summary */}
          {(results.successful.length > 0 || results.failed.length > 0 || results.skipped.length > 0) && (
            <div className="bg-white shadow-sm rounded-lg overflow-hidden p-4">
              ...
              <ul className="text-sm text-gray-600">
                <li>Successful: {results.successful.length} contacts labeled</li>
                <li>Failed: {results.failed.length} contacts</li>
                <li>Skipped: {results.skipped.length} contacts (already labeled or have Person Info page)</li>
                {results.retried && results.retried.length > 0 && (
                  <li>Retried: {results.retried.length} contacts (successfully recovered)</li>
                )}
              </ul>
            </div>
          )}
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full overflow-hidden shadow-xl">
            <div className="px-6 py-4">
              <h3 className="text-lg font-medium text-gray-900">Confirm Labeling Process</h3>
              <p className="mt-2 text-sm text-gray-500">
                Add 'custom.hasnoinfopage' label to {contactsToLabel.length} contacts?
              </p>
              <div className="mt-3 bg-gray-50 p-3 rounded-md">
                <p className="text-xs text-gray-700 font-medium">Operation details:</p>
                <ul className="mt-1 text-xs">
                  <li className="text-green-600">{contactsToLabel.length} contacts will be updated</li>
                </ul>
              </div>
            </div>
      ...
      {showResumeDialog && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full overflow-hidden shadow-xl">
            <div className="px-6 py-4">
              <h3 className="text-lg font-medium text-gray-900">Resume Previous Process?</h3>
              <p className="mt-2 text-sm text-gray-500">
                A previous labeling process was interrupted ({progress.completed} of {progress.total} contacts processed).
