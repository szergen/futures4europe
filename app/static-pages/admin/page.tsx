...
const AdminContactsPanel = () => {
  ...
  const [bulkActionResults, setBulkActionResults] = useState<{ 
    successful: string[], failed: string[], skipped: string[] 
  ...
  useEffect(() => {
    if (infoPages?.length > 0) {
      ...
      console.log('Sample info page full structure:', JSON.stringify(infoPages[0], null, 2));
      ...
      const pagesWithNestedSlug = infoPages.filter(page => 
        page.data?.slug || 
  ...
  const handleLabelOperation = async (contactId: string, action: BulkAction) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        notify(`Label ${action === 'addLabel' ? 'added' : 'removed'} successfully`, 'success');
          ...
          if (contact._id === contactId) {
            ...
            return {
              ...
              info: {
                ...contact.info,
                labelKeys: {
                  items: action === 'addLabel'
                    ? [...currentLabels, 'custom.hasnoinfopage']
                    : currentLabels.filter(label => label !== 'custom.hasnoinfopage')
                }
              }
            };
          }
        ...
        notify(data.message || `Failed to ${action === 'addLabel' ? 'add' : 'remove'} label`, 'error');
      }
    } catch (error: any) {
  ...
  const toggleSelectContact = (contactId: string) => {
    ...
    newSelectedIds.has(contactId) ? newSelectedIds.delete(contactId) : newSelectedIds.add(contactId);
  ...
  const toggleSelectAll = () => {
    selectedContactIds.size === filteredContacts.length
      ...
      : setSelectedContactIds(new Set(filteredContacts.map(contact => contact._id)));
  };
  ...
  const performBulkAction = async () => {
    ...
    const processContact = async (contactId: string, retryCount = 0): Promise<{ 
      success: boolean, skipped?: boolean, contactId: string, error?: string, retried?: boolean
    }> => {
      try {
        ...
        if ((action === 'addLabel' && hasLabel) || (action === 'removeLabel' && !hasLabel)) {
        ...
        const response = await fetch('/api/members', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ contactId, action, revision: contact.revision }),
        });
        ...
        results.failed.push(contactId);
        return { success: false, contactId, error: (error as Error).message, retried: retryCount > 0 };
      }
    };
    ...
    setContacts(prevContacts => {
      return prevContacts.map(contact => {
        if (results.successful.includes(contact._id)) {
          ...
          if (action === 'addLabel' && !currentLabels.includes('custom.hasnoinfopage')) {
            return {
              ...contact,
              info: {
                ...contact.info,
          ...
          } else if (action === 'removeLabel' && contact.info.labelKeys?.items) {
            return {
              ...contact,
              info: {
                ...contact.info,
                labelKeys: { 
                  items: contact.info.labelKeys.items.filter(label => label !== 'custom.hasnoinfopage')
                }
              }
            };
          }
    ...
    if (results.failed.length > 0) statusMessage += ` Failed for ${results.failed.length} contacts.`;
    if (results.skipped.length > 0) statusMessage += ` Skipped ${results.skipped.length} contacts (already updated).`;
    if (results.retried.length > 0) statusMessage += ` ${results.retried.length} operations required retries.`;
    
    notify(statusMessage.trim(), results.failed.length > 0 ? 'error' : 'success', 15000);
  ...
  const getInfoPageUrl = useCallback((contact) => {
    ...
    if (shouldDebug) {
      console.log(`Looking for info page for: "${fullName}" (ID: ${contact._id})`);
    }
  ...
  const hasLabelInconsistency = useCallback((contact) => {
  ...
  const filterContact = useCallback((contact: Contact) => {
    ...
    if (searchQuery.trim() !== '') {
      ...
      const fullName = `${firstName} ${lastName}`.trim();
      const email = contact.info.emails?.items?.[0]?.email?.toLowerCase() || '';
    ...
    if (filters.hasNoInfoPage && !contact.info.labelKeys?.items?.includes('custom.hasnoinfopage')) {
      return false;
    }
    
    if (filters.isMember && contact.source?.sourceType !== 'WIX_SITE_MEMBERS') {
      return false;
    }
    
    if (filters.emailSubscribed) {
      const primaryStatus = contact.primaryEmail?.subscriptionStatus;
      const extendedStatus = contact.info.extendedFields?.items?.['emailSubscriptions.subscriptionStatus'];
      const isSubscribed = (primaryStatus === 'SUBSCRIBED') || (extendedStatus === 'SUBSCRIBED');
    ...
    if (filters.hasPersonInfoPage) {
      ...
      const hasPersonInfoPage = infoPages.some(page => {
        ...
        if (titleMatches) {
          ...
          const pageHasPersonInfoType = page.data?.pageTypes?.some(pageType => 
        ...
        if (page.data?.person && Array.isArray(page.data.person)) {
          const personMatch = page.data.person.some(personTag => 
            personTag.name?.toLowerCase() === contactFullName
          );
    ...
    return true;
  }, [searchQuery, filters, infoPages, pageTypes, getInfoPageUrl, hasLabelInconsistency]);
  ...
  const calculateBulkActionStats = useCallback(() => {
    ...
    selectedContactIds.forEach(id => {
      ...
      if ((pendingBulkAction === 'addLabel' && !hasLabel) || 
  ...
  useEffect(() => {
    if (contacts.length > 0 && infoPages.length > 0) {
      ...
      if (inconsistentContacts.length > 0) {
        console.log(`Found ${inconsistentContacts.length} contacts with label inconsistency`);
          ...
          <div className="bg-white shadow-sm rounded-lg overflow-hidden mb-6">
            <div className="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
              <div className="flex items-center space-x-2">
                ...
                />
                <label htmlFor="select-all" className="mb-0 text-sm font-medium text-gray-700">
                  {selectedContactIds.size === 0 
                    ? "Select All" 
              ...
              <div className="flex space-x-2">
                <button
                  ...
                  disabled={isBulkActionInProgress || selectedContactIds.size === 0}
          ...
          {isBulkActionInProgress && (
            <div className="bg-blue-50 border border-blue-200 rounded-md p-4 mb-6">
              ...
              <p className="text-xs text-blue-800">
                {bulkProgress.completed} of {bulkProgress.total} contacts processed
              </p>
            </div>
          )}
            ...
            <div className="px-4 py-5 sm:p-6">
              {isLoading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="mt-2 text-sm font-medium text-gray-900">No contacts found</p>
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        <input
                          ...
                          disabled={isBulkActionInProgress || actionInProgress === contact._id}
                        ...
                        <div>
                            ...
                            <span className={`ml-2 px-2 py-0.5 text-xs rounded-full ${
                              contact.source?.sourceType === 'WIX_SITE_MEMBERS' 
                                ...
                                : (contact.info?.extendedFields?.items && 
                                  contact.info.extendedFields.items['members.membershipStatus'] === 'APPROVED')
                          ...
                          <p className="text-sm text-gray-500">
                            {contact.info.emails?.items?.[0]?.email || 'No email'}
                          </p>
...
{contact.primaryEmail && (
  <p className="text-xs text-gray-600">
    Subscription: {contact.primaryEmail.subscriptionStatus}
  </p>
)}
...
{getInfoPageUrl(contact) && (
  <div className="mt-1">
    <Link href={getInfoPageUrl(contact)} target="_blank">
      <button 
      ...
      >
        ...
        View Info Page
                      ...
                      <div>
                        {contact.info.labelKeys?.items?.includes('custom.hasnoinfopage') ? (
                          <button
                            onClick={() => handleLabelOperation(contact._id, 'removeLabel')}
                            disabled={isBulkActionInProgress || actionInProgress === contact._id || selectedContactIds.size > 0}
                            className={`px-4 py-2 rounded-md text-sm font-medium text-white ${
                              isBulkActionInProgress || actionInProgress === contact._id || selectedContactIds.size > 0
                                ? 'bg-red-300 cursor-not-allowed'
                                : 'bg-red-600 hover:bg-red-700'
                            }`}
                          >
                            {actionInProgress === contact._id ? 'Processing...' : 'Remove Label'}
                          </button>
                        ) : (
                          <button
                            onClick={() => handleLabelOperation(contact._id, 'addLabel')}
                            disabled={isBulkActionInProgress || actionInProgress === contact._id || selectedContactIds.size > 0}
                            className={`px-4 py-2 rounded-md text-sm font-medium text-white ${
                              isBulkActionInProgress || actionInProgress === contact._id || selectedContactIds.size > 0
                                ? 'bg-green-300 cursor-not-allowed'
      ...
      <div className="px-6 py-4">
        ...
        <div className="mt-3 bg-gray-50 p-3 rounded-md">
          ...
          <ul className="mt-1 text-xs">
            <li className="text-green-600">{bulkStats.actionable} contacts will be updated</li>
            {bulkStats.skippable > 0 && (
              <li className="text-amber-600">
                {bulkStats.skippable} contacts will be skipped (already {pendingBulkAction === 'addLabel' ? 'have' : 'don\'t have'} the label)
              </li>
            )}
          </ul>
        </div>
